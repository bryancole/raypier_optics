...
class PSF(HasTraits):
    ...
    def resample_at_exit_pupil(self, rays):
        owner = self.owner
        mask = rays.length != numpy.Infinity
        directions = rays.direction[mask]
        E1 = rays.E1_amp
        E2 = rays.E2_amp
        amp = numpy.sqrt(E1*E1.conjugate() + E2*E2.conjugate())[:,numpy.newaxis]
        ...
        aspect = dotprod(rays.direction, mean_dir)
        
        new_origin = rays.origin - (h/aspect) * rays.direction
